...
1. 어셈블리를 강력한 이름으로 서명
1. HangulSyllable 형식 추가
1. 메서드 이름 변경
   1. IsHangul → IsHangulSyllable
   1. IsJamo → IsHangulJamo
   1. ToChar → ToJamo
1. 상수 이름 변경
   1. FirstChar → FirstSyllable
   1. LastChar → LastSyllable
1. 메서드 추가
...
* 2.0-beta.2 에서의 변경 사항
1. HangulSyllable을 Serializable로 표시
1. WrappedChar 속성을 Value로 변경
